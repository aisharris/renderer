...
void Surface::intersectBVH(uint32_t nodeIdx, Ray& ray, Interaction& si)
{
    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            Tri curr = this->tris[this->getIdx(i + node.firstPrim)];
            Interaction siIntermediate = this->rayTriangleIntersect(ray, curr.v1, curr.v2, curr.v3, curr.normal);

            Barycentric(siIntermediate,  curr.v1,  curr.v2,  curr.v3,  curr);
            
            if(this->hasDiffuseTexture())
            {
                siIntermediate.diffuse = 1;
            }
            else
            {
                siIntermediate.diffuse = 0;
            }
...
// Compute barycentric coordinates (alpha, beta, gamma) for
// point p with respect to triangle (a, b, c)
void Barycentric(Interaction &si, Vector3f a, Vector3f b, Vector3f c, Tri curr)
{
    Vector3f v0 = b - a, v1 = c - a, v2 = si.p - a;
    float d00 = Dot(v0, v0);
    float d01 = Dot(v0, v1);
    float d11 = Dot(v1, v1);
    float d20 = Dot(v2, v0);
    ...
    if (u >= 0 && v >= 0 && w >= 0) 
    {
        si.uv_intersect = (u * curr.uv1) + (v * curr.uv2) + (w * curr.uv3);
    }
}